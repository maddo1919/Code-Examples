...
static void	load_all_texture(t_conf **conf)
{
	load_texture(*conf, &(*conf)->avatar, "textures/bob.xpm");
...
void	init_window(t_conf *conf)
{
	conf->mlx = mlx_init();
	open_window(conf);
	conf->img.addr = mlx_get_data_addr(conf->img.img, conf->img.bits_per_pixel, &conf->img.line_length, &conf->img.endian);
